...
Suite Teardown    BuiltIn.Run Keywords    AppiumLibrary.Close Application    AND    commonKeyword.Stop Appium Server
*** Test Cases ***
...
TC-03 Add product to cart
    ...
    BuiltIn.Set suite variable    ${price_product}    ${price}

TC-04 Click on cart icon
    mainPage.Tap cart icon in main page

TC-05 Verify that in cart has only one product
    cartFeature.Verify product that in cart    ${product.amount}    ${price_product}    ${product.backpack}